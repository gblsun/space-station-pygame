...
pontos = [projetar(transformar(v, objeto['pos'], objeto['escala'], objeto['rot']),
camera) for v in CUBO]
for a, b in ARESTAS:
...
objeto['pos'], objeto['rot'], objeto['escala'] = [0,0,8], [0,0,0],
[1.5,1.5,1.5]
filho['pos'], filho['rot'], filho['escala'] = [0,0,2.8], [0,0,0],
[0.55,0.55,0.55]
teclas = pygame.key.get_pressed()
alvo = objeto if selecionado == 'pai' else filho
velocidade = 3 * dt
# Translação do objeto selecionado no plano XZ.
if teclas[pygame.K_LEFT]: alvo['pos'][0] -= velocidade
if teclas[pygame.K_RIGHT]: alvo['pos'][0] += velocidade
if teclas[pygame.K_UP]: alvo['pos'][2] -= velocidade
if teclas[pygame.K_DOWN]: alvo['pos'][2] += velocidade
# Rotação em torno dos eixos X e Y.
if teclas[pygame.K_a]: alvo['rot'][1] -= 1.8 * dt
if teclas[pygame.K_d]: alvo['rot'][1] += 1.8 * dt
if teclas[pygame.K_w]: alvo['rot'][0] -= 1.8 * dt
if teclas[pygame.K_s]: alvo['rot'][0] += 1.8 * dt
# Escala uniforme.
if teclas[pygame.K_q]: alvo['escala'] = [max(0.2, v-1.2*dt) for v in alvo['escala']]
if teclas[pygame.K_e]: alvo['escala'] = [v+1.2*dt for v in alvo['escala']]
# Manipulação da câmera: I/K sobe/desce, J/L desloca lateralmente.
if teclas[pygame.K_j]: camera[0] -= 3*dt
if teclas[pygame.K_l]: camera[0] += 3*dt
if teclas[pygame.K_i]: camera[1] += 3*dt
if teclas[pygame.K_k]: camera[1] -= 3*dt
tela.fill((18, 25, 38))
...
texto = f"Selecionado: {selecionado} | TAB alterna | Setas move | A/D e W/S gira |
Q/E escala"
